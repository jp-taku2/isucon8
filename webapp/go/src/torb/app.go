...
func getEvent(eventID, loginUserID int64) (*Event, error) {
	...
	rows, err := db.Query("SELECT * FROM reservations LEFT JOIN sheets ON sheets.id = reservations.sheet_id WHERE event_id = ? AND canceled_at IS NULL GROUP BY event_id, sheet_id HAVING reserved_at = MIN(reserved_at)", event.ID)
...
func unreserveEvent(c echo.Context) error {
	eventID, err := strconv.ParseInt(c.Param("id"), 10, 64)
	if err != nil {
		return resError(c, "not_found", 404)
	}
...
func getEventsAsAdmin(c echo.Context) error {
	events, err := getEvents(true)
	if err != nil {
		return err
	}
...
func getEventAsAdmin(c echo.Context) error {
	eventID, err := strconv.ParseInt(c.Param("id"), 10, 64)
	if err != nil {
		return resError(c, "not_found", 404)
	}
...
func editEventAsAdmin(c echo.Context) error {
	eventID, err := strconv.ParseInt(c.Param("id"), 10, 64)
	if err != nil {
		return resError(c, "not_found", 404)
	}
...
func getSalesByEvent(c echo.Context) error {
	eventID, err := strconv.ParseInt(c.Param("id"), 10, 64)
	if err != nil {
		return resError(c, "not_found", 404)
	}
...
func main() {
	...
	e.GET("/admin/api/events", getEventsAsAdmin, adminLoginRequired)
	e.POST("/admin/api/events", addEvent, adminLoginRequired)
	e.GET("/admin/api/events/:id", getEventAsAdmin, adminLoginRequired)
	e.POST("/admin/api/events/:id/actions/edit", editEventAsAdmin, adminLoginRequired)
	e.GET("/admin/api/reports/events/:id/sales", getSalesByEvent, adminLoginRequired)
	e.GET("/admin/api/reports/sales", getSales, adminLoginRequired)
